...
export type Metric = 'all' | 'realtime';
...
export async function getStats(
  ...
  const [realtime, totals, previous, timeseries, pages, sources, countries, devices, browsers, operatingSystems] =
    await Promise.all([
      ...
      breakdown(domain, w.start, w.end, 'pathname', 10),
      breakdown(domain, w.start, w.end, 'referrer_src', 10),
      breakdown(domain, w.start, w.end, 'country', 10),
      breakdown(domain, w.start, w.end, 'device', 5),
      breakdown(domain, w.start, w.end, 'browser', 8),
      breakdown(domain, w.start, w.end, 'os', 8),
    ]);
